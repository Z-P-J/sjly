...
public class FillImageView extends ImageView {
    ...
    public FillImageView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //未选中显示的Drawable
        Drawable unCheckDrawable = typedArray.getDrawable(R.styleable.FillImageView_unCheckImage);
        if (unCheckDrawable != null) {
            setImageDrawable(unCheckDrawable);
        }
        //选中显示的Drawable
        Drawable checkDrawable = typedArray.getDrawable(R.styleable.FillImageView_checkImage);
    ...
    private void init() {
        ...
        mPaint.setColor(Color.WHITE); // Color.parseColor("#f2f2f2")
        mPath = new Path();
    }
...
//        }
    ...
    public void setCheckedImage(@DrawableRes int res) {
        Drawable checkDrawable = getResources().getDrawable(res);
        BitmapDrawable bitmapDrawable = (BitmapDrawable) checkDrawable;
        if (bitmapDrawable != null) {
            mCheckBitmap = bitmapDrawable.getBitmap();
            invalidate();
