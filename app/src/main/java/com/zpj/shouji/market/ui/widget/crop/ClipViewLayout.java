...
public class ClipViewLayout extends RelativeLayout {
    ...
    private ClipView.ClipType clipType = ClipView.ClipType.CIRCLE;
...
//        int clipType = array.getInt(R.styleable.ClipViewLayout_clipType, 1);
     ...
     * 初始化图片
     */
    public void setImageSrc(final File file) {
     ...
     */
    public void initSrcPic(File file) {
        if (file == null) {
            return;
        }
        String path = file.getAbsolutePath();
//        Log.d("evan", "**********clip_view uri*******  " + file);
...
//        }
        ...
        if (bitmap.getWidth() >= bitmap.getHeight()) {//宽图
            ...
            RectF rect = clipView.getClipRect();
            //高的最小缩放比
            minScale = rect.height() / bitmap.getHeight();
            if (scale < minScale) {
                scale = minScale;
            }
        } else {//高图
            ...
            RectF rect = clipView.getClipRect();
            //宽的最小缩放比
            minScale = rect.width() / bitmap.getWidth();
            if (scale < minScale) {
                scale = minScale;
            }
        }
        ...
        //imageView的中心x
        float midX = imageView.getWidth() / 2f;
        //imageView的中心y
        float midY = imageView.getHeight() / 2f;
        //bitmap的中心x
        float imageMidX = ((float) bitmap.getWidth() * scale / 2);
        //bitmap的中心y
        float imageMidY = ((float) bitmap.getHeight() * scale / 2);
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mode == DRAG) { //拖动
                    ...
                    Log.d("ClipViewLayout", "dx=" + dx + " dy=" + dy + " getScale=" + getScale());
                    //检查边界
                    checkBorder();
    ...
    private void checkBorder() {
        ...
        Log.d("ClipViewLayout", "checkBorder rect.width()=" + rect.width() + " width=" + width + " rect.width() >= width - 2 * mHorizontalPadding=" + (rect.width() >= width - 2 * mHorizontalPadding));
        if (rect.width() >= width - 2 * mHorizontalPadding - 0.01) {
            if (rect.left >= mHorizontalPadding) {
                deltaX = -rect.left + mHorizontalPadding;
            }
            Log.d("ClipViewLayout", "checkBorder deltaX1=" + deltaX);
            if (rect.right <= width - mHorizontalPadding) {
                deltaX = width - mHorizontalPadding - rect.right;
            }
            Log.d("ClipViewLayout", "checkBorder deltaX2=" + deltaX);
        }
        if (rect.height() >= height - 2 * mVerticalPadding - 0.01) {
    ...
    public Bitmap clip() {
        ...
        try {
            cropBitmap = Bitmap.createBitmap(imageView.getDrawingCache(), (int) rect.left, (int) rect.top, (int) rect.width(), (int) rect.height());
            int width = 900;
            int height;
